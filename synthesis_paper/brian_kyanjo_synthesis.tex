...
\begin{document}
	...
	\donna{Motivate paper with general tsunami papers (George, LeVeque, Popinet. ) 
	Applications to tsunamis, flooding, other natural hazards. All of these numerical tsunami models rely on solving Riemann problems which can robustly track a wetting and drying interfaces to model runup on beaches and inunudation into harbors and communities along the affected coastlines.  Various approaches have been taken to model handle these wetting and drying states.  These include approaches taken by George \cite{ge:2008,ge:2011}, Popinet \cite{po:2015, po:2018}, Liu et al. \cite{li-ta-wa-ca-ba-ch-li:2021}, 
	Pelanti \cite{pe-bo-ma:2011}, Bunya et al \cite{bu-ku-we-da:2009}, Toro   }

\ignore{	
	\section{Riemann solver for SWE}
	Riemann solvers are the numerical method in which time-averaged fluxes of all conserved quantities are calculated to solve fundamental problems in conservation laws named Riemann problems.   A Riemann problem can be defined as a specific initial value problem  (Cauchy) of a partial differential equation (PDE) that consists of conservation equations \eqref{rp0}  combined with piece-wise constant initial data which has a single discontinuity in the domain of interest as shown in equation\eqref{rp1}\cite{ge:2011}. 
	\begin{eqnarray}
		{}q_{t} + f(q)_{x}& =& 0
		\label{rp0}\\
		q(x,0)& =& \begin{cases}
...
} % End ignore for bathymetry section
...
} % Ignore details of fwave approach. 
	\section{Limiters}
	\donna{Don't include this section}
\ignore{	
...
\ignore{
	...
	\donna{Include this section after your discussion of SWE, Riemann Problems and the Finite Volume method.   Show several examples.  You can compare your exact solver results to WPA results, for example.  Or show first and second order results to set these solvers in context.  I wouldnt' mention much about limiters.  But describe the structure.}  
