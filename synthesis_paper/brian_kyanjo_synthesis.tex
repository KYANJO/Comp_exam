...
\section{Beach run-up and inundation}
	...
	\donna{Describe why we want to modeling the wetting and drying front (beach runup; inundation).   explain what can go wrong (negative height values from \ref{cellupdat}}

	\donna{.Can we just set $h = \epsilon$ whenever $h < 0$?}

The wet/dry Riemann solution for the right, middle and left dry state is depicted in figures~\ref{fig:right}, \ref{fig:middle}, and \ref{fig:left}.
